...
export function StatusTick({ message }: { message: ChatMessage }) {
  ...
  if (message.status === 'failed') {
    return (
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--danger-ink)" strokeWidth={2.2} strokeLinecap="round">
  ...
  if (message.readAt) {
    return (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--accent-ink)" strokeWidth={2.2} strokeLinecap="round" strokeLinejoin="round">
...
export function UploadBar({ message }: { message: ChatMessage }) {
  ...
  return (
    <div style={css('height:3px;border-radius:2px;background:rgba(11,95,239,.16);overflow:hidden')}>
      <div style={{ height: '100%', width: `${Math.round((message.progress ?? 0) * 100)}%`, background: 'var(--accent)', transition: 'width .15s linear' }} />
    </div>
  );
}
