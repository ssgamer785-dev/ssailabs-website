...
export function CandleChart({ stamp, seed = 7 }: { stamp?: string; seed?: number }) {
  ...
  return (
    <div style={css('position:absolute;inset:0;background:var(--ink-chip-3);overflow:hidden')}>
      <div style={css('position:absolute;inset:0;background:radial-gradient(120% 90% at 20% 0%,rgba(37,99,235,.13),transparent 60%)')} />
      <div style={css('position:absolute;left:0;right:24px;top:20%;height:1px;background:rgba(var(--faint-rgb),.09)')} />
      <div style={css('position:absolute;left:0;right:24px;top:45%;height:1px;background:rgba(var(--faint-rgb),.09)')} />
      <div style={css('position:absolute;left:0;right:24px;top:70%;height:1px;background:rgba(var(--faint-rgb),.09)')} />
      <div style={css('position:absolute;left:0;right:24px;top:8%;bottom:6%')}>
        {bars.map((b, i) => {
          const left = i * slot + (slot - bw) / 2;
          const col = b.up ? 'var(--success)' : 'var(--danger)';
          return (
            <div key={i}>
      ...
      <div style={css('position:absolute;top:0;right:0;bottom:0;width:24px;border-left:1px solid rgba(var(--faint-rgb),.12);background:rgba(8,13,26,.6);display:flex;flex-direction:column;justify-content:space-around;align-items:center;padding:6px 0')}>
        <div style={css('width:9px;height:1px;background:rgba(var(--faint-rgb),.3)')} />
        <div style={css('width:9px;height:1px;background:rgba(var(--faint-rgb),.3)')} />
        <div style={css('width:13px;height:6px;border-radius:1.5px;background:var(--success-deep)')} />
        <div style={css('width:9px;height:1px;background:rgba(var(--faint-rgb),.3)')} />
        <div style={css('width:9px;height:1px;background:rgba(var(--faint-rgb),.3)')} />
        <div style={css('width:9px;height:1px;background:rgba(var(--faint-rgb),.3)')} />
      </div>
