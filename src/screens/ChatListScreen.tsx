      ...
      <div className="nav-space" style={css('flex:1;min-height:0;padding:6px 20px 0;display:flex;flex-direction:column;overflow-y:auto')}>
        <div onClick={() => navigate('/chat/admin')} style={css('display:flex;align-items:center;gap:12px;padding:11px 0;cursor:pointer')}>
          <div style={css('position:relative;flex:none')}>
            ...
            <div style={css('position:absolute;right:0;bottom:0;width:12px;height:12px;border-radius:50%;background:var(--success);border:2.4px solid var(--border-on-accent)')} />
          </div>
        ...
        <div style={css('height:1px;background:var(--surface-divider)')} />
        <div style={css('margin-top:20px;background:var(--surface-inset);border-radius:14px;padding:16px 15px;display:flex;flex-direction:column;align-items:center;gap:8px')}>
          <div style={css('width:42px;height:42px;border-radius:13px;background:var(--surface);display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(var(--shadow-rgb),.06)')}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--text-faint)" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><rect x="5" y="10.4" width="14" height="9.6" rx="2.6" /><path d="M8.2 10.4V8a3.8 3.8 0 0 1 7.6 0v2.4" /></svg>
          </div>
          <div style={css('font-size:13px;font-weight:700;letter-spacing:-.2px;text-align:center')}>Direct member chat is off</div>
          <div style={css("font-size:11.5px;color:var(--text-muted);line-height:1.5;text-align:center;text-wrap:pretty")}>Members cannot message each other. Post in the Students Community or write to the Admin instead.</div>
        </div>
