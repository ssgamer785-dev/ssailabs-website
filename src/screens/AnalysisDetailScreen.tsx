...
const AVATAR_TINTS: [string, string][] = [
  ['var(--avatar-bg)', 'var(--avatar-ink)'], ['var(--avatar-bg-3)', 'var(--avatar-ink-3)'],
  ['var(--avatar-bg-5)', 'var(--avatar-ink-5)'], ['var(--avatar-bg-4)', 'var(--avatar-ink-4)'],
];
...
function Comment({ initial, bg, fg, name, body, time, onDelete }: { initial: string; bg: string; fg: string; name: string; body: string; time: string; onDelete?: () => void }) {
  return (
    <div onContextMenu={e => { if (onDelete) { e.preventDefault(); onDelete(); } }} style={css('margin-top:8px;background:var(--surface-inset);border-radius:12px;padding:11px 12px;display:flex;align-items:flex-start;gap:10px')}>
      ...
      <div style={css('flex:1;display:flex;flex-direction:column;gap:3px;min-width:0')}>
        ...
        <div style={css('font-size:12.5px;color:var(--text-muted)')}>{body}</div>
      </div>
      <div style={css('font-size:10.5px;color:var(--text-faint);flex:none;white-space:nowrap')}>{time}</div>
    </div>
  );
}
    ...
    <PhoneShell>
      ...
      <div style={css('flex:1;min-height:0;padding:4px 20px 0;display:flex;flex-direction:column;overflow-y:auto')}>
        <div style={css('display:flex;align-items:center;gap:10px')}>
          ...
          <div style={css('font-size:11.5px;color:var(--text-faint);flex:none;white-space:nowrap')}>2h ago</div>
        </div>
        <div style={css('margin-top:14px;font-size:16px;font-weight:700;letter-spacing:-.35px')}>{post?.title ?? 'Gold Analysis'}</div>
        <div style={css('margin-top:6px;font-size:13.5px;line-height:1.5;color:var(--text-secondary);white-space:pre-wrap')}>{post?.body ?? 'Buy Above 3365'}</div>
        ...
        <div style={css('margin-top:13px;display:flex;gap:10px')}>
          <Hoverable style={css('height:36px;padding:0 14px;border:1px solid var(--border-5);border-radius:10px;display:flex;align-items:center;gap:7px;cursor:pointer;background:var(--surface)')} hoverStyle={css('border-color:var(--accent)')}>
        ...
        <div style={css('margin-top:14px;display:flex;align-items:center;gap:18px')}>
          <div onClick={toggleLike} style={css('display:flex;align-items:center;gap:7px;cursor:pointer')}>
            <svg width="20" height="20" viewBox="0 0 24 24" style={css('display:block')}><path d="M12 20.8S3.9 15.4 3.9 9.9A4.55 4.55 0 0 1 12 7.1a4.55 4.55 0 0 1 8.1 2.8c0 5.5-8.1 10.9-8.1 10.9z" fill={liked ? 'var(--danger-ink)' : 'none'} stroke={liked ? 'var(--danger-ink)' : 'var(--text-muted)'} strokeWidth={1.7} strokeLinejoin="round" /></svg>
            <div style={css('font-size:13px;font-weight:600;color:var(--text-secondary)')}>{likes}</div>
          </div>
          <div style={css('display:flex;align-items:center;gap:7px;cursor:pointer')}>
            ...
            <div style={css('font-size:13px;font-weight:600;color:var(--text-secondary)')}>{comments.comments.length}</div>
          </div>
          <div style={css('flex:1')} />
          <div onClick={() => setBm(v => !v)} style={css('cursor:pointer')}>
            <svg width="19" height="19" viewBox="0 0 24 24" style={css('display:block')}><path d="M7 4.4h10v16l-5-3.5-5 3.5z" fill={bm ? 'var(--accent-ink)' : 'none'} stroke={bm ? 'var(--accent-ink)' : 'var(--text-muted)'} strokeWidth={1.7} strokeLinejoin="round" /></svg>
          </div>
        ...
        {comments.loading ? (
          <div style={css('margin-top:10px;font-size:12.5px;color:var(--text-faint)')}>Loading comments…</div>
        ) : comments.comments.length === 0 ? (
          <div style={css('margin-top:10px;font-size:12.5px;color:var(--text-faint)')}>No comments yet — be the first to reply.</div>
        ...
        {comments.hasMore && (
          <div onClick={comments.loadMore} style={css('margin-top:10px;text-align:center;font-size:12px;font-weight:600;color:var(--accent-ink);cursor:pointer')}>
      ...
      {!rec ? (
        <div style={{ ...css('flex:none;padding:12px 18px 26px;display:flex;align-items:center;gap:9px'), paddingBottom: 'calc(10px + var(--nav-space))' }}>
          <div style={css('width:36px;height:36px;border-radius:50%;background:var(--surface-secondary);display:flex;align-items:center;justify-content:center;cursor:pointer;flex:none')}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth={2.1} strokeLinecap="round"><path d="M12 6v12M6 12h12" /></svg>
          </div>
          ...
          <div onClick={toggleRec} title="Hold to record a voice message" style={css('width:36px;height:36px;border-radius:50%;flex:none;cursor:pointer;display:flex;align-items:center;justify-content:center;background:var(--surface-secondary)')}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round"><rect x={9} y={3.2} width={6} height={10.4} rx={3} /><path d="M5.6 11.4a6.4 6.4 0 0 0 12.8 0M12 17.8v3M8.8 20.8h6.4" /></svg>
          </div>
          <Hoverable onClick={sendComment} style={css('width:40px;height:40px;border-radius:50%;background:var(--accent-soft-2);display:flex;align-items:center;justify-content:center;cursor:pointer;flex:none')} hoverStyle={css('background:var(--accent-soft-3)')}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="var(--accent-ink)" style={css('margin-left:-1px')}><path d="M20.8 3.2 3.9 9.9c-.7.3-.6 1.3.1 1.5l6.3 1.9 1.9 6.3c.2.7 1.2.8 1.5.1z" /></svg>
          </Hoverable>
        </div>
      ) : (
        <div style={css('flex:none;padding:12px 18px 26px;display:flex;align-items:center;gap:9px')}>
          <Hoverable onClick={toggleRec} style={css('width:36px;height:36px;border-radius:50%;background:var(--danger-soft);display:flex;align-items:center;justify-content:center;cursor:pointer;flex:none')} hoverStyle={css('background:var(--danger-soft-4)')}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="var(--danger-ink)" strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round"><path d="M5.6 7.4h12.8M9.4 7.4V5.2h5.2v2.2M7.2 7.4l.9 12h7.8l.9-12" /></svg>
          </Hoverable>
          <div style={css('flex:1;height:42px;border-radius:999px;background:var(--danger-soft-2);border:1px solid var(--danger-border);display:flex;align-items:center;padding:0 14px;gap:10px;min-width:0')}>
            <div style={css('width:8px;height:8px;border-radius:50%;background:var(--danger);flex:none')} />
            <div style={css('font-size:12.5px;font-weight:700;color:var(--danger-ink);flex:none;white-space:nowrap')}>{clock(recSec)}</div>
            <Wave bars={30} color="rgba(239,68,68,.55)" height={20} gap={2.4} seed={57} />
          </div>
          <div onClick={toggleRec} title="Hold to record a voice message" style={css('width:36px;height:36px;border-radius:50%;flex:none;cursor:pointer;display:flex;align-items:center;justify-content:center;background:var(--danger);box-shadow:0 0 0 4px rgba(239,68,68,.16)')}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--on-accent)" strokeWidth={1.9} strokeLinecap="round" strokeLinejoin="round"><rect x={9} y={3.2} width={6} height={10.4} rx={3} /><path d="M5.6 11.4a6.4 6.4 0 0 0 12.8 0M12 17.8v3M8.8 20.8h6.4" /></svg>
          </div>
          <Hoverable style={css('width:40px;height:40px;border-radius:50%;background:var(--accent-soft-2);display:flex;align-items:center;justify-content:center;cursor:pointer;flex:none')} hoverStyle={css('background:var(--accent-soft-3)')}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="var(--accent-ink)" style={css('margin-left:-1px')}><path d="M20.8 3.2 3.9 9.9c-.7.3-.6 1.3.1 1.5l6.3 1.9 1.9 6.3c.2.7 1.2.8 1.5.1z" /></svg>
          </Hoverable>
        </div>
      )}
